...
| Wave | Observe named environments in caller-provided order, or admit one durable content-bound coordinator that applies through existing plans. | Historical (2026-07-29): observation only, no durable wave record. Current: durable execute/status/stop/resume/rollback (`docs/rollout-waves.md`, ADR 0017). `wave run` still observes only. | `wave run` never applies. `wave execute` applies through existing plans, approvals, gates, and rollback. |
