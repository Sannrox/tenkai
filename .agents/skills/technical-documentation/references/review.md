...
- If a repository-specific overlay exists under `references/`, apply it for page type, docs IA, preservation, examples, and validation checks.
...
- When a docs site config or index exists (for example `docs.json`, mkdocs.yml, sidebar config), check nav routing, main path versus reference placement, and generated-reference visibility.
- For rewrites or page splits, require source-backed keep/drop/move/destination coverage for important claims, warnings, examples, commands, fields, and troubleshooting facts.
...
- Confirm the content matches an explicit page type (overview, quickstart, topic, guide, reference, troubleshooting, governance).
